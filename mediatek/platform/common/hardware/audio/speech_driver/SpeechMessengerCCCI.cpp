...
void *SpeechMessengerCCCI::CCCIReadThread(void *arg)
{
    ...
    while (pCCCI->CCCIEnable) {
        ...
        switch (m2a_message_id) {
            ...
            case M2A_VB_PLAY_STARTED:
            {
                // caorongxing add start for PTT
                system("am broadcast -a 'com.android.phone.kptt_voicebuffering_state_changed' -e 'KPTT_VB_STATE' 'VB_PLAY_STARTED'");
                // caorongxing add end
                ALOGE("KNPTT[INFO]:YYQ M2A_VB_PLAY_STARTED");
                break;
            }
            case M2A_VB_PLAY_COMPLETED:
            {
               // pCCCI->mHw->voiceBufferDisable();
                // caorongxing add start for PTT
                system("am broadcast -a 'com.android.phone.kptt_voicebuffering_state_changed' -e 'KPTT_VB_STATE' 'VB_PLAY_COMPLETED'");
                // caorongxing add end
                ALOGE("KNPTT[INFO]:YYQ M2A_VB_PLAY_COMPLETED");
                break;
            }
            case M2A_VB_PLAY_ABORTED:
            {
               // pCCCI->mHw->voiceBufferDisable();
                // caorongxing add start for PTT
                system("am broadcast -a 'com.android.phone.kptt_voicebuffering_state_changed' -e 'KPTT_VB_STATE' 'VB_PLAY_ABORTED'");
                // caorongxing add end
                ALOGE("KNPTT[INFO]:YYQ M2A_VB_PLAY_ABORTED");
                break;
            }
            case M2A_VB_PLAY_ERROR:
            {
                // caorongxing add start for PTT
                system("am broadcast -a 'com.android.phone.kptt_voicebuffering_state_changed' -e 'KPTT_VB_STATE' 'VB_PLAY_ERROR'");
                // caorongxing add end
                ALOGE("KNPTT[INFO]:YYQ M2A_VB_PLAY_ERROR");
                break;
            }
...
/*porting for ALPS00712639(For_JRDHZ72_WE_JB3_ALPS.JB3.MP.V1_P18)*/
            ...
            case MSG_M2A_VIBSPK_PARAMETER_ACK: { // ack of MSG_M2A_VIBSPK_PARAMETER
               /*susheng.ye.hz merge JRD475758(For_JRDHZ72_WE_JB3_ALPS.JB3.MP.V1_P20) 20130624  begin*/
                pCCCI->A2MBufUnLock();
              /*susheng.ye.hz merge JRD475758(For_JRDHZ72_WE_JB3_ALPS.JB3.MP.V1_P20) 20130624  end*/
