...
/******************************************************
Rule:
ro.audio.parameters.config=		description
-------------------------------------------------------
		0					US, no HD voice, Default
		
		1					US, no HD voice, Latam
		
		2					EU, HD voice, Default
		
		3					EU, no HD voice, Default
		
		4					US, Normal Receiver, Digicel
*******************************************************/
...
bool isDualMicSupport()
{
	int is_support;
	char value[PROPERTY_VALUE_MAX];
	//property_get("ro.dual_mic", value, "0");
	//return (atoi(value) == 1) ? true:false;
	property_get("ro.audio.parameters.config", value, "0");
	is_support = atoi(value);
	ALOGD("is_support = %d", is_support);
	if(is_support == 2)  // EU, HD voice, Default
		return true;
	else
		return false;
}
...
bool isUsing2in1Speaker()
{
        int is_using;
	char value[PROPERTY_VALUE_MAX];
	//property_get("ro.2in1spk", value, "0");
	//return (atoi(value) == 1) ? true:false;
	property_get("ro.audio.parameters.config", value, "0");
	is_using = atoi(value);
	ALOGD("is_using = %d", is_using);
	if(is_using == 2 || is_using == 4)
		return false;
	else
		return true;
}
