...
static int adev_voicebuffer_start(struct audio_hw_device *dev, int duration)
{
    struct a2dp_audio_device *ladev = to_ladev(dev);
    return ladev->hwif->startVoiceBuffer(duration);
}
static int adev_voicebuffer_stop(struct audio_hw_device *dev)
{
    struct a2dp_audio_device *ladev = to_ladev(dev);
    return ladev->hwif->stopVoiceBuffer();
}
static int adev_voicebuffer_play(struct audio_hw_device *dev)
{
    struct a2dp_audio_device *ladev = to_ladev(dev);
    return ladev->hwif->playVoiceBuffer();
}
static int adev_voicebuffer_clean(struct audio_hw_device *dev)
{
    struct a2dp_audio_device *ladev = to_ladev(dev);
    return ladev->hwif->cleanVoiceBuffer();
}
static int adev_voicebuffer_disable(struct audio_hw_device *dev)
{
    struct a2dp_audio_device *ladev = to_ladev(dev);
    return ladev->hwif->voiceBufferDisable();
}
static int adev_voicebuffer_abnormal(struct audio_hw_device *dev)
{
    struct a2dp_audio_device *ladev = to_ladev(dev);
    return ladev->hwif->dealWithAbnormalCallHangup();
}
static int adev_voicebuffer_init(struct audio_hw_device *dev)
{
    struct a2dp_audio_device *ladev = to_ladev(dev);
    return ladev->hwif->initStartVoiceBufferWithoutPTTCall();
}
//caorongxing add end for PTT
    //-------------------------------------------------------------------------
                              ...
                              hw_device_t **device)
    {
        ...
        // caorongxing add start for PTT
        ladev->device.startVoiceBuffer = adev_voicebuffer_start;
        ladev->device.stopVoiceBuffer = adev_voicebuffer_stop;
        ladev->device.playVoiceBuffer = adev_voicebuffer_play;
        ladev->device.cleanVoiceBuffer = adev_voicebuffer_clean;
        ladev->device.voiceBufferDisable = adev_voicebuffer_disable;
        ladev->device.dealWithAbnormalCallHangup = adev_voicebuffer_abnormal;
        ladev->device.initStartVoiceBufferWithoutPTTCall = adev_voicebuffer_init;
        // caorongxing add end for PTT
