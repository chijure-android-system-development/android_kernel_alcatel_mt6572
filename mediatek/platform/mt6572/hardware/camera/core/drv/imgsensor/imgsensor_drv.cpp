...
ImgSensorDrv::impSearchSensor(pfExIdChk pExIdChkCbf)
{
	...
	int active_id=0xEE;//zhao.li@tcl add for Bug 495381
    ...
    for (SensorEnum = DUAL_CAMERA_MAIN_SENSOR; SensorEnum <= DUAL_CAMERA_SUB_SENSOR; SensorEnum <<= 1)  {
        ...
        for (i = 0; i < MAX_NUM_OF_SUPPORT_SENSOR; i++) {
			...
			if(i==active_id) continue;//zhao.li@tcl add for Bug 495381
        ...
        //zhao.li@tcl add for Bug 495381
        if(err==0)//search the active sensor 
        	{
        	active_id=i;
			break;
        	}
