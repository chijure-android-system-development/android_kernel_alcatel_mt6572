...
// Added for FR590070 begin by yi.zheng.hz@tcl.com
#include "LoopbackManager.h"
// Added for FR590070 end by yi.zheng.hz@tcl.com
...
status_t AudioLoopbackController::OpenAudioLoopbackControlFlow(const audio_devices_t input_device, const audio_devices_t output_device, uint32 ul_samplerate, uint32 dl_samplerate)
{
    ...
    if (output_device == AUDIO_DEVICE_OUT_SPEAKER) {
        ...
        // Modified for FR590070 begin by yi.zheng.hz@tcl.com begin
        loopback_t loopback_type = LoopbackManager::GetInstance()->GetLoopbackTypeNoLock();
        ALOGD("loopback type = %d", loopback_type);
	if(loopback_type == AP_REF_MIC_AFE_LOOPBACK && 
		output_device == AUDIO_DEVICE_OUT_EARPIECE) // Sub mic loopback && earpiece output
	{
	...
	else
	{
        ...
        // Modified for FR590070 begin by yi.zheng.hz@tcl.com end
    }
#endif
