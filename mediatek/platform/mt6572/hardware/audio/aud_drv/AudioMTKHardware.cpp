...
// caorongxing add start
    mPcm4Way_Enable = false;
    mPTT_Stuts = false;
// caorongxing add end
    return NO_ERROR;
}
...
// start/stop the pcm4way interface for java invoked
// caorongxing add start
status_t AudioMTKHardware::startVoiceBuffer(int duration)
{
  ALOGD("KNPTT: caorongxing: startVoiceBuffer()");
//  mMicMute_ModemStatus = false;
  mPcm4Way_Enable = true;
  mAudioResourceManager->EnableAudioClock(AudioResourceManagerInterface::CLOCK_AUD_ADC, true);  
  mPTT_Stuts = true;
  audio_devices_t output_device = (audio_devices_t)mAudioResourceManager->getDlOutputDevice();
  audio_devices_t input_device  = (audio_devices_t)mAudioResourceManager->getUlInputDevice(); 
  ALOGD("KNPTT: mInputAvailable=0x%x, output_device=0x%x", input_device, output_device);
  if ((input_device == AUDIO_DEVICE_IN_BLUETOOTH_SCO_HEADSET)
      /*|| (input_device & android_audio_legacy::AudioSystem::DEVICE_OUT_BLUETOOTH_SCO_CARKIT)
      || (input_device & android_audio_legacy::AudioSystem::DEVICE_OUT_BLUETOOTH_SCO)*/) 
  {
    ALOGD("KNPTT: INPUT is bluetooth");
    //pLad->LAD_SetSpeechMode(SPH_MODE_BT_EARPHONE);
    //pLad->LAD_SetInputSource(SetInputSource(LADIN_BTIn));
    //mVolumeController->SetLadMicGain(Headset_Mic);
    if (mSpeechDriverFactory->GetSpeechDriver()->GetApSideModemStatus(SPEECH_STATUS_MASK) == false)
    {
        ALOGD("KNPTT: call SpeechOn()");
        mSpeechDriverFactory->GetSpeechDriver()->SetSpeechMode(input_device, output_device);
        mAudio_Control_State.bSpeechFlag = true;
        UpdateKernelState();
        mSpeechDriverFactory->GetSpeechDriver()->SpeechOn();
    }
    mSpeechDriverFactory->GetSpeechDriver()->LAD_PCM4WayOn();
  } else if (input_device == AUDIO_DEVICE_IN_WIRED_HEADSET)
  {
    ALOGD("KNPTT: INPUT is wired headset");
///////////////////
...
///////////////////
  ...
  {
    ALOGD("KNPTT: INPUT is normal mic");
///////////////////
...
///////////////////
  ...
  //SetMicGain();
  //pLad->LAD_SetInputSource(SetInputSource(LADIN_Microphone1));
...
status_t AudioMTKHardware::stopVoiceBuffer()
{
  ALOGD("KNPTT: caorongxing: stopVoiceBuffer()");
  mSpeechDriverFactory->GetSpeechDriver()->LAD_VoiceBufferStop();
  return NO_ERROR;
}

status_t AudioMTKHardware::playVoiceBuffer()
{
  ALOGD("KNPTT: caorongxing: playVoiceBuffer()");
//  doOutputDeviceRouting();
  mSpeechDriverFactory->GetSpeechDriver()->LAD_VoiceBufferPlay();
  return NO_ERROR;
}

status_t AudioMTKHardware::cleanVoiceBuffer()
{
//  pLad->LAD_VoiceBufferClean();
  //pLad->LAD_VoiceBufferStop();          // test for add the stop PCM4Way interface
  ALOGD("KNPTT: caorongxing: cleanVoiceBuffer()");
  mSpeechDriverFactory->GetSpeechDriver()->LAD_PCM4WayOff();
  mPcm4Way_Enable = false;
  mSpeechDriverFactory->GetSpeechDriver()->LAD_VoiceBufferClean();
  return NO_ERROR;
}

status_t AudioMTKHardware::voiceBufferDisable()
{
  ALOGD("KNPTT: Voice buffer disable by modem");
  mPTT_Stuts = false;
  return NO_ERROR;
}

status_t AudioMTKHardware::dealWithAbnormalCallHangup()
{
  ALOGD("KNPTT: The call is abnormity hangup!!!!!!!!!!!!!!");
  mSpeechDriverFactory->GetSpeechDriver()->LAD_PCM4WayOff();
  if (mSpeechDriverFactory->GetSpeechDriver()->GetApSideModemStatus(SPEECH_STATUS_MASK) == true)
  {
      mSpeechDriverFactory->GetSpeechDriver()->SpeechOff();
  }
...
status_t AudioMTKHardware::initStartVoiceBufferWithoutPTTCall()
{
  ALOGD("KNPTT: When the first boot init start voicebuffering but no ptt call !");
  mSpeechDriverFactory->GetSpeechDriver()->LAD_PCM4WayOff();
  if (mSpeechDriverFactory->GetSpeechDriver()->GetApSideModemStatus(SPEECH_STATUS_MASK) == true)
  {
      mSpeechDriverFactory->GetSpeechDriver()->SpeechOff();
...
//zhao.li@tcl atv
#ifdef NMI5625	
    mAudioDigitalInstance->SetMATVDigitalStatus(enable); //zhao.li@tcl atv
#endif	
//zhao.li@tcl atv end
    if (enable == true) {
...
status_t AudioMTKHardware::doSetMode()
{
    ...
    switch (mMode) {
        case AUDIO_MODE_NORMAL: {
            switch (mNextMode) {
                ...
                case AUDIO_MODE_IN_CALL_2: {      // Normal->Incall2: MD2 MO call dial out
                    ALOGD("KNPTT: caorongxing:doSetMode, AUDIO_MODE_NORMAL->%d", mNextMode);
        ...
        case AUDIO_MODE_RINGTONE: {
            switch (mNextMode) {
                ...
                case AUDIO_MODE_IN_CALL_2: {      // Ringtone->Incall2: Accept MD2 MT call
                    ALOGD("KNPTT: caorongxing:doSetMode, AUDIO_MODE_RINGTONE->%d", mNextMode);
                    pSpeechPhoneCallController->OpenModemSpeechControlFlow(mNextMode);
                    break;
                }
        ...
        case AUDIO_MODE_IN_CALL_2: {
         // caorongxing add start
         // comment below code by yeqing , PCM4Way off will never be called , if call process is PTT->GSM->Hung up 
         //if (mPTT_Stuts == true) // if ptt call , turn off the pcm4way
          {
          if(mPcm4Way_Enable == true)
           {
             ALOGD("KNPTT: caorongxing:doSetMode, call off then the pcm4way turn off!");
             mSpeechDriverFactory->GetSpeechDriver()->LAD_PCM4WayOff();
             //cleanVoiceBuffer(); // caorongxing test for pcm4wayoff in cleanvoicebuffer TODO
             mSpeechDriverFactory->GetSpeechDriver()->LAD_VoiceBufferClean();
             mPcm4Way_Enable = false;
           }
           mPTT_Stuts = false ;
          // LOG_HARDWARE("KNPTT: caorongxing:call off , set ptt_stuts = false");
          }
         // caorongxing add end
            switch (mNextMode) {
        ...
        case AUDIO_MODE_IN_COMMUNICATION: {
            switch (mNextMode) {
                ...
                case AUDIO_MODE_IN_CALL_2: {      // Incommunication->Incall2: MD2 Dail out & Hold SIP call, or Hang up SIP call and back to MD2
                    ALOGD("KNPTT: caorongxing:doSetMode, AUDIO_MODE_IN_COMMUNICATION->%d", mNextMode);
                    pSpeechPhoneCallController->OpenModemSpeechControlFlow(mNextMode);
                    break;
                }
            }
