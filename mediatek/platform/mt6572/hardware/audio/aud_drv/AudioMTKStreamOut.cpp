...
/* Removed code for Porting JRD519743(For_JRDHZ72_WE_JB3_ALPS.JB3.MP.V1_P83) */
                if(VUnlockhdl != NULL)
                {
...
#ifdef NMI5625				 
				 || mAudioDigitalControl->GetMATVDigitalStatus()//zhao.li@tcl atv
#endif				 
				 ) 
    {
            mAudioResourceManager->SelectOutputDevice(new_device);
...
//modify for 2in1 speaker cust by yi.zheng.hz begin
    ...
    else if((mbUsing2in1Speaker)&&(current_device & AUDIO_DEVICE_OUT_EARPIECE))
    {
        SetStreamOutCompFltStatus(AUDIO_COMP_FLT_VIBSPK,true);
        SetStreamOutCompFltStatus(AUDIO_COMP_FLT_HEADPHONE,false);
        SetStreamOutCompFltStatus(AUDIO_COMP_FLT_AUDENH,false);
