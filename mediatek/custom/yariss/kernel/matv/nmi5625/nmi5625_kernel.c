...
#include <linux/proc_fs.h>  // zhao.li@tcl atv
...
#define NMI_POWER_VCORE_PIN            GPIO23 //GPIO70//0xFF //75  //   VCORE_1.2  

#define NMI_RESET_PIN               GPIO30 //GPIO78//0xFF //30  

#define NMI_ATV_ANT_PIN				GPIO134//zhao.li@tcl ATV
#define NMI_FM_ANT_PIN				GPIO136
...
static ssize_t store_ant(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (count == size) {
		...
		if (state != s_ant_status) {
			s_ant_status = state;
			if (s_ant_status > 0) {
				mt_set_gpio_out(NMI_ATV_ANT_PIN, 1);
			} else {
				mt_set_gpio_out(NMI_ATV_ANT_PIN, 0);
			}
		}
	}
	return ret;
...
static DEVICE_ATTR (ant, 0666, show_ant, store_ant);
...
static int nmi5625_open(struct inode *inode, struct file *file)
{
	...
	mt_set_gpio_out(NMI_ATV_ANT_PIN, 0);// zhao.li@tcl atv
	...
	mt_set_gpio_out(NMI_FM_ANT_PIN, 1);// zhao.li@tcl atv
		    ...
		    unsigned int cmd, unsigned long arg)
{
	...
	switch ((cmd&0xffff0000)) {
		...
		if (arg == 1) {	/* on */
			...
			mt_set_gpio_out(NMI_FM_ANT_PIN, 0);//zhao.li@tcl atv
			mt_set_gpio_out(NMI_ATV_ANT_PIN, 1);//zhao.li@tcl atv
		}
		break;
