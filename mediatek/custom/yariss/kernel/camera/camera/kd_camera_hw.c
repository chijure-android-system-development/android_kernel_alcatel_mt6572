...
//zhao.li@tcl add for Hi257 Bug 466785 end
		...
		msleep(5);
	   	...
	   	PK_DBG("current open camera is gc0328 sensor\n\r");
       		if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800,mode_name))//IO VDD
		{
			PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");		            
			goto _kdCISModulePowerOn_exit_;
...
//	            }
	   	...
	   	PK_DBG("current open camera is sp0718 sensor\n\r");
		if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_A, VOL_2800,mode_name))//AVDD
		{
		      	PK_DBG("[CAMERA SENSOR] Fail to enable analog power\n");
		      	goto _kdCISModulePowerOn_exit_;
...
/*yi.zheng.hz modify for FR437080 start*/
       ...
       //}
#if 0//zhao.li@tcl open this for sub camera //remove GC0329 code
	if((0 == strcmp(SENSOR_DRVNAME_GC0328_YUV,currSensorName)) || (0 == strcmp(SENSOR_DRVNAME_GC0329_YUV,currSensorName)))
	{       
...
#endif
	{
	       if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_D, mode_name)) {
	         ...
	         //zhao.li@tcl  goto _kdCISModulePowerOn_exit_;
	       }
