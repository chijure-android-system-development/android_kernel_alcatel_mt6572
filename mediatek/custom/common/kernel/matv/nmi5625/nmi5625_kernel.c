...
#define NMI_ATV_ANT_PIN				GPIO135
		    ...
		    unsigned int cmd, unsigned long arg)
{
	...
	case NM5625_PWR_1P2_CTL:
		...
		if (arg == 1) {	/* on */
			...
			printk("%s GPIO_CAMERA_CMPDN1_PIN:%d, status:%d\n", __func__, GPIO_CAMERA_CMPDN1_PIN, mt_get_gpio_out(GPIO_CAMERA_CMPDN1_PIN));
			///
			...
			if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_A, "nmitv")) {
				g_bIsAtvStart = false;	//qiangbo.ni add for PR613196 sub camera no preview 2014-03-03
				goto _fail_;
			}
