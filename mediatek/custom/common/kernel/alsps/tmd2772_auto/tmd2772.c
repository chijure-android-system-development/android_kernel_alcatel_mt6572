...
//#ifdef TARGET_BUILD_MMITEST  /*only open in MiniSW version*/
#define SHOW_PS_VALUE_IN_MINISW
//#endif
#ifdef SHOW_PS_VALUE_IN_MINISW
static int boot_ps_value=0;  //save the power on ps rawdata vaule.
#endif
...
static struct platform_driver tmd2772_alsps_driver;
  ...
  #endif//#if defined(MTK_AUTO_DETECT_PLSENSOR)  added by zhengdao
	  ...
	  case I2C_FLAG_READ:
	     ...
	     res = i2c_master_send(client, buf, count);
	       ...
	       if(res <= 0)
	          ...
	          res = i2c_master_recv(client, &buf[1], 0x1);
	     break;
	  default:
...
//get adc channel 0 value
	...
	res = tmd2772_i2c_master_operate(client, buffer, 0x1, I2C_FLAG_READ);
...
long tmd2772_read_ps(struct i2c_client *client, u16 *data)
{
	...
	if(res <= 0)
	{
		...
		ps_value_high[0]=buffer[1];

	*data = ps_value_low[0] | (ps_value_high[0]<<8);
	printk("zhengdao_ps_data=%d,\n", *data);
	...
	for(j = 0;j<M;j++)
	 ...
	 if(i == M)
	 {
		...
		if(i == cali_num)
		{
			...
			//BEGIN: fangjie add for show the PS values before or after calibration in MMITest ALSPS test item. 
			#ifdef SHOW_PS_VALUE_IN_MINISW
			boot_ps_value = sum/cali_num; //save the rawdata for MMITest alsps test item
			#endif
			//END: fangjie add for show the PS values before or after calibration in MMITest ALSPS test item. 
...
GET_THRE:
			...
			printk("zhengdao_ps_data_cali_close=%d, zhengdao_ps_data_cali_faraway=%d\n", ps_data_cali->close, ps_data_cali->far_away);
			printk("zhengdao_ps_cali_close=%d, zhengdao_ps_cali_faraway=%d\n", ps_cali.close, ps_cali.far_away);
...
/* cat /sys/bus/platform/drivers/als_ps/alsvalue */
...
static ssize_t show_ps_current_value(struct device_driver *ddri, char *buf)
{
	int len = 0;
	u16 ps_act = 0;
	tmd2772_read_ps(tmd2772_i2c_client,&ps_act);//---------------------------llf
...
{
	int len = 0;
	printk("fangjie: boot_ps_value:%d; offset: %d,; close:%d; faraway:%d\n",boot_ps_value,ps_cali.offset,ps_cali.close, ps_cali.far_away);
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			{				
				...
				//APS_LOG("111*\n");
				tmd2772_init_client_for_cali(obj->client);
				tmd2772_read_data_for_cali_for_call(obj->client,&ps_cali, 6);  //Note: We do not use first ps read data
				//APS_LOG("222*\n");
#endif
...
//BEGIN: fangjie add for show the PS values before or after calibration in MMITest ALSPS test item. 
#ifdef SHOW_PS_VALUE_IN_MINISW
	struct device_driver *alsps_driver = NULL; //create device_driver.
#if defined(MTK_AUTO_DETECT_ALSPS)
...
printk("tmd2772_probe");
 ...
 #endif
 printk("liang_tmd2772_local_init");
	return 0;
}
/*----------------------------------------------------------------------------*/
static int tmd2772_remove(struct platform_device *pdev)
...
static int __init tmd2772_init(void)
{
	...
	i2c_register_board_info(1, &i2c_tmd2772, 1); //i2c bus1;
 		...
 		printk("%s:MTK_AUTO_DETECT_ALSPS=%d 1111\n",__func__,MTK_AUTO_DETECT_ALSPS);
 #else
