...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_show_alsvalue(struct device_driver *ddri, char *buf)
{
	int res;
...
#endif
/*----------------------------------------------------------------------------*/
static DRIVER_ATTR(alsvalue,     S_IWUSR | S_IRUGO, stk3x1x_show_alsvalue,   NULL);
...
static int stk3x1x_get_als_value(struct stk3x1x_priv *obj, u16 als)
{
	...
	for(idx = 0; idx < obj->als_level_num; idx++)
	{
		if(als < obj->hw->als_level[idx])
		{
			...
			case 0: //<5
				if(idx<idx_old){
					if(als<(obj->hw->als_level[idx]-2)){
						break;
					}else{
						idx=idx_old;//als_level don't change;
					}
				}else if(idx>idx_old){
					if(als>(obj->hw->als_level[idx]+10)){
						break;
					}else{
						idx=idx_old;//als_level don't change;
					}
			...
			case 5://<190
				if(idx<idx_old){
					if(als<(obj->hw->als_level[idx]-40)){
						break;
					}else{
						idx=idx_old;//als_level don't change;
					}
				}else if(idx>idx_old){
					if(als>(obj->hw->als_level[idx]+60)){
						break;
					}else{
						idx=idx_old;//als_level don't change;
					}
