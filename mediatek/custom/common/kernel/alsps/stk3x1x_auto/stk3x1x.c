...
#define STK_TUNE0
#define STK_FIR
#define STK_ALS_DEBOUNCE   //added by zhengdao for untibounce algorithem
//#define STK_IRS
#define STK_FIR_LEN	16     //changed average num
...
#define STK3211_STK3311_PID	0x12
	...
	#define STK_HT_N_CT	        100          // 160                            //83
	#define STK_LT_N_CT	        65         // 35                            //50
#endif /* #ifdef STK_TUNE0 */

#ifdef STK_ALS_DEBOUNCE                  // added by zhengdao
	#define STK_ALS_CHANGE_THRESHOLD	3
...
#ifdef STK_FIR
struct data_filter {
    u16 raw[MAX_FIR_LEN];     //changed by zhengdao
    u32 sum;
    int num;
    int idx;
};
...
#ifdef STK_ALS_DEBOUNCE    //added by zhengdao for remember last lever
	u16 als_last;
#endif	
};
/*----------------------------------------------------------------------------*/
...
//BEGIN: fangjie add for show the PS values before or after calibration in MMITest ALSPS test item. 
//#ifdef TARGET_BUILD_MMITEST  /*only exist in MINISW*/
#define STK_MINISW_SHOW
//#endif
#if defined(STK_MINISW_SHOW)
static int boot_ps_value=-1;
#endif
//END: fangjie add for show the PS values before or after calibration in MMITest ALSPS test item. 

/*----------------------------------------------------------------------------*/
int stk3x1x_get_addr(struct stk3x1x_alsps_hw *hw, struct stk3x1x_i2c_addr *addr)
{
	if(!hw || !addr)
	{
		return -EFAULT;
...
int stk3x1x_read_als(struct i2c_client *client, u16 *data)
{
	...
	else
	{
...
#ifdef STK_FIR
		if(obj->fir.num < STK_FIR_LEN)    //changed by zhengdao
		{                
		...
		{
			idx = obj->fir.idx % STK_FIR_LEN;    //changed by zhengdao
			obj->fir.sum -= obj->fir.raw[idx];
			obj->fir.raw[idx] = als_data;
			obj->fir.sum += als_data;
			obj->fir.idx++;
			als_data = obj->fir.sum/STK_FIR_LEN;   //changed by zhengdao
		}	
#endif
	}
...
#endif
/*----------------------------------------------------------------------------*/
static void stk3x1x_power(struct stk3x1x_alsps_hw *hw, unsigned int on) 
{
...
static int stk3x1x_enable_als(struct i2c_client *client, int enable)
{
	...
	if(enable)
	{
#ifdef STK_ALS_DEBOUNCE        //added by zhengdao for als_last initial value
		obj->als_last = 0;
#endif
...
*/		
	...
	if(enable)
	{
		...
		obj->psa = 0;				 //added by zhengdao , make every time calibrate
		obj->psi = 0xFFFF;			 //added by zhengdao
...
//BEGIN: fangjie add for show the PS values before or after calibration in MMITest ALSPS test item. 
		#if defined(STK_MINISW_SHOW)
		boot_ps_value = obj->ps_stat_data[1];
		#endif
//END: fangjie add for show the PS values before or after calibration in MMITest ALSPS test item. 
		stk_ps_tune_zero_final(obj);
	}		
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	if(obj->psi > obj->hw->ps_high_thd_val)
	{
		atomic_set(&obj->ps_high_thd_val, obj->hw->ps_high_thd_val ); 
		atomic_set(&obj->ps_low_thd_val, obj->hw->ps_low_thd_val ); 
		...
		printk("%s: FAE tune0 psi(%d)> hw->ps_high_thd_val(%d)\n", __func__, obj->psi, obj->hw->ps_high_thd_val);
	    hrtimer_cancel(&obj->ps_tune0_timer);
		return 0;
	}
	...
	{
		//   obj->psi_set = obj->psi;         // cancel by zhengdao , make every time calibrate
...
int stk3x1x_setup_eint(struct i2c_client *client)
{
  ...
  APS_LOG("ALS/PS interrupt pin = %d\n", EINT2_PS_N);		
	...
	mt_set_gpio_pull_enable(EINT2_PS_N, GPIO_PULL_ENABLE);
...
static ssize_t show_ps_current_value(struct device_driver *ddri, char *buf)
{
	int res;
	if(!stk3x1x_obj)
	{
	...
	res = stk3x1x_read_ps(stk3x1x_obj->client, &stk3x1x_obj->ps);
      ...
      // not consider failed stuation
      return scnprintf(buf, PAGE_SIZE, "%d\n", stk3x1x_obj->ps);     

}

/* cat /sys/bus/platform/drivers/als_ps/ps_current */
static DRIVER_ATTR(ps_current,       S_IWUSR | S_IRUGO, show_ps_current_value, NULL);

static ssize_t show_ps_status_value(struct device_driver *ddri, char *buf)
{
	int len = 0;
	...
	len = snprintf(buf, PAGE_SIZE, "%d:%d:%d\n", 
		boot_ps_value, atomic_read(&stk3x1x_obj->ps_high_thd_val), atomic_read(&stk3x1x_obj->ps_low_thd_val));
	return len;
}

/* cat /sys/bus/platform/drivers/als_ps/ps_status */
static DRIVER_ATTR(ps_status,       S_IWUSR | S_IRUGO, show_ps_status_value, NULL);
#endif
//END: fangjie add for show the PS values before or after calibration in MMITest ALSPS test item. 
/*----------------------------------------------------------------------------*/
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	struct stk3x1x_priv *obj = (struct stk3x1x_priv *)self;
	u8 flag;
#ifdef STK_ALS_DEBOUNCE   //added by zhengdao for diff value
	u16 als_diff = 0;
#endif
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			{				
				...
				{
#ifdef STK_ALS_DEBOUNCE     //added by zhengdao for the algorithem
					if(obj->als >= obj->als_last)
					{
						als_diff = obj->als - obj->als_last;
					}
					else
					{
						als_diff = obj->als_last - obj->als;
					}
					...
					else if(als_diff > STK_ALS_CHANGE_THRESHOLD)
					{
						obj->als_last = obj->als;
						sensor_data->values[0] = stk3x1x_get_als_value(obj, obj->als);
					}
					...
					APS_LOG("%s: als=%d, als_last=%d, als_diff=%d, Lux_values=%d\n", __FUNCTION__,obj->als,obj->als_last,als_diff,sensor_data->values[0]);
#else
					sensor_data->values[0] = stk3x1x_get_als_value(obj, obj->als);
#endif
					sensor_data->value_divide = 1;
...
/*----------------------------------------------------------------------------*/
 ...
 #endif
{
	struct stk3x1x_alsps_hw *hw = stk3x1x_get_cust_alsps_hw();
	struct stk3x1x_i2c_addr addr;
 ...
 #endif
 printk("liang_stk3x1x_local_init");
	return 0;
}
/*----------------------------------------------------------------------------*/
static int stk3x1x_remove(struct platform_device *pdev)
