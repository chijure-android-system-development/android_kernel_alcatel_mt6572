...
static unsigned char CTPM_FW_CAMRY_MUTTO[]=
{
	...
	#include "YarisS_ID53_V1a_20140419_app.i"
};
...
static unsigned char CTPM_FW_YARIS_JUNDA[]=
{
	...
	#include "YarisS_ID85_V35_20140424_app.i"
...
int fts_ctpm_auto_upg(unsigned char tp_vendor)
{
	...
	unsigned char version_list_camry_mutto[] = {0x0,0x10,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1a};  //Keep the Mutto tp firmware old version list that allows to be updated.
	...
	unsigned char version_list_camry_junda[] = {0x0,0x10,0x11,0x30,0x32,0x33,0x34,0x35};	//Keep the ft6306 tp firmware old version list that allows to be updated.
	int i,i_ret = -1;
	...
	if(tp_vendor == 0x5a){
		...
		{
			if((uc_tp_fm_ver<uc_host_fm_ver) || uc_tp_fm_ver==0x11)//llf 2013626
			//if(1) //force to up
			{
...
/********************************Upgrade Firmware Function end****************************************/
	...
	static int ft6306_tp_resume_flag=0;//add by PR653624
#endif

static void ft6306_tp_reset() //add by PR653624
{
...
/*************************************************************************************/
static int  ft6306_ps_mode_enable(bool enable)
{	      
	int err = 0;
	unsigned char rx_data;

	ft6306_tp_resume_flag=0;//add by PR653624
	if(enable)  //enter phone open
	{
		TPD_DMESG("[FT6306]TPD wake up befor the PS enable!**************************\n");
		ft6306_tp_reset(); 
		...
		if(err>=0 && (rx_data&0x01==1)){
			...
			TPD_DMESG("[FT6306_PS]open ps failed,rx_data=%d err=%d \n",rx_data,err);
			return -1;
		}
	}
	else //close phone
	{     
		...
		if(err>=0 && (rx_data ==0)){
			...
			TPD_DMESG("[FT6306_PS]close ps failed,  rx_data=%d err=%d\n",rx_data,err);
		}
	}
	return 0;
}
...
int ft6306_pls_enable(void)
{
	...
	if(ft6306_pls_opened>0){
		TPD_DMESG("[FT6306_PS]ft6306_pls have enable\n");
		return 0;
	}
	ret=ft6306_ps_mode_enable(true);
	if(ret>=0){
		ft6306_pls_opened = 1;
	}
	return 0;
...
int ft6306_pls_disable(void)
{
	...
	if(ft6306_pls_opened<1){
		TPD_DMESG("[FT6306_PS]ft6306_pls have disable\n");
		return 0;
	}
...
force_detect:
 ...
 static void tpd_resume( struct early_suspend *h )
 {
	...
	if(ft6306_pls_opened)
	{	
		if(ft6306_tp_resume_flag==0){
			ft6306_tp_reset();
		}
		ft6306_ps_mode_enable(true);//opened by PR653624
 ...
 static int tpd_suspend(struct i2c_client *client)
 {
	...
	TPD_DMESG("[FT6306] TPD enter sleep******************************************\n");
...
#else
	TPD_DMESG("[FT6306]TP enter sleep mode_____________________________________\n");
