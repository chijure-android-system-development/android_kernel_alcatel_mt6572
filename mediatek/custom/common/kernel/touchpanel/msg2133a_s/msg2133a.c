...
{
  #include "YarisS_Each_V506_20131219.h"
};
...
/*reset the chip*/
	...
	static int msg2133_tp_resume_flag=0;//add by PR653624
#endif
...
static int  msg2133_ps_mode_enable(bool enable)
{
	int err = 0;
	unsigned char dbbus_tx_data[4];
	unsigned char dbbus_rx_data[4];
	TPD_DMESG("pls  wuyongtao_ctp ctp ps msg2133a_ps_mode_enable\n");
	msg2133_tp_resume_flag=0;
	if(enable)
	{
	      TPD_DEBUG("[MSG2133A]Before the PS enable wake up the CTP\n");
 ...
 {
 	int ret=0;
	 TPD_DMESG("ap3212c_pls_enable\n");
	 if(mstar2133_pls_opened>0){
		TPD_DMESG("[MSG2133a] msg2133a_pls have enabled\n");
		return 0;
	 }
	 ret=msg2133_ps_mode_enable(true);
	 if(ret>=0){
 ...
 {
	 TPD_DMESG("<6> wuyongtao ==>ap3212c_pls_disable");
	  if(mstar2133_pls_opened<1){
		TPD_DMESG("[MSG2133a] msg2133a_pls have disabled\n");
		return 0;
	 }
...
static int Upgrade_detect()
{
 ...
 static int tpd_resume(struct i2c_client *client)
 {
	...
	if(mstar2133_pls_opened)
		{
			if(msg2133_tp_resume_flag==0){ //force to reset the PS will be FAR
				_HalTscrHWReset();
			}
			msg2133_ps_mode_enable(true);//opened by PR653624
