...
extern void mtk_wdt_restart(enum wk_wdt_type type);//add by rongxiao.deng for avoid watchdog timeout
...
int g_v_magnify_x_gt = TPD_VELOCITY_CUSTOM_X;
int g_v_magnify_y_gt = TPD_VELOCITY_CUSTOM_Y;
                               ...
                               unsigned long arg)
{
    ...
    switch (cmd)
    {
        case TPD_GET_VELOCITY_CUSTOM_X:
            ...
            if (copy_to_user(data, &g_v_magnify_x_gt, sizeof(g_v_magnify_x_gt)))
            {
                err = -EFAULT;
                break;
            }
        ...
        case TPD_GET_VELOCITY_CUSTOM_Y:
            ...
            if (copy_to_user(data, &g_v_magnify_y_gt, sizeof(g_v_magnify_y_gt)))
            {
                err = -EFAULT;
                break;
            }
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	mtk_wdt_restart(WK_WDT_EXT_TYPE_NOLOCK);//add by rongxiao.deng for avoid watchdog timeout
  ...
  #if 1                             //  #ifdef MT6572   changed by zhengdao
...
//	mt65xx_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN, CUST_EINT_TOUCH_PANEL_POLARITY, tpd_eint_interrupt_handler, 1); //changed by zhengdao 
...
//*************add hard reset function*******************//
          GTP_INFO("IC WORKS ABNORMALLY!  RESETTING GTUIAR");
          ret=-1;
          u8 hard_reset[6]={0x42,0x26,0x01,0x01,0x01};
           u8 j;
          for(j=0;j<5;j++)
          {
		ret=gtp_i2c_write(i2c_client_point,hard_reset,5);
		if(ret<0)
		{
                  GTP_ERROR("gtp hard reset fail,j=%d",j);
		}
            else
             {
		    GTP_INFO("gtp hard reset success,j=%d",j);
		    break;
             }
